...
class SavedTournamentsList extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      nextProps.storageChangesListener !== this.props.storageChangesListener
    ) {
      this.setState({
        savedTournamentsList: StorageManager.getElement("tournaments")
      });
    }
  }

  componentDidMount() {
    StorageManager.getElement("tournaments") &&
  ...
  deleteTournamentItem = () => {
    ...
    const updatedList = savedTournaments.filter(
...
export default connect(
  ...
  { setStorageChanges, setModalStatus }
)(SavedTournamentsList);
